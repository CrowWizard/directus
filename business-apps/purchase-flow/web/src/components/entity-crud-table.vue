				...
				<tbody>
					<tr v-for="item in items" :key="item.id">
						...
						<td class="row-actions">
							<button type="button" class="ghost-button" :data-test="`edit-${item.id}`" @click="edit(item)">编辑</button>
							<button type="button" class="danger-button" :data-test="`delete-${item.id}`" @click="$emit('delete', item.id)">
								删除
							</button>
						</td>
					</tr>
