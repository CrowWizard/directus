...
HOOK_DIR="${EXTENSIONS_PATH}/purchase-flow-auto-id"
HOOK_DIST_DIR="${HOOK_DIR}/dist"
HOOK_FILE="${HOOK_DIST_DIR}/index.js"
PUBLIC_APP_URL="${PUBLIC_APP_URL:-http://localhost:8055}"
ACCEPT_TOKEN_TTL_HOURS="${ACCEPT_TOKEN_TTL_HOURS:-24}"

mkdir -p "${HOOK_DIST_DIR}"
...
cat > "${HOOK_DIR}/package.json" <<'EOF'
{
  "name": "purchase-flow-auto-id",
  "version": "1.0.0",
  "type": "module",
  "directus:extension": {
    "type": "hook",
...
echo "已安装 Hook：${HOOK_FILE}"
echo "功能：自动 UUID、自动分配采购员、设置超时提醒时间、生成接单 Token、可选企业微信通知。"
...
echo "如需 Hook 直接发企业微信，请给 Directus 服务进程配置 WECHAT_WORK_WEBHOOK_URL。"
