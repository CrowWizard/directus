...
for collection in customers customer_contacts inquiry_items customer_quotes conversations attachments supplier_quotes suppliers priority_rules assignment_accept_tokens; do
  grant_all_actions "${SALES_POLICY_ID}" "${collection}"
done

for collection in suppliers supplier_contacts inquiry_items supplier_quotes conversations attachments buyer_profiles assignment_rules priority_rules customers assignment_accept_tokens; do
  grant_all_actions "${BUYER_POLICY_ID}" "${collection}"
done

for collection in customers customer_contacts suppliers supplier_contacts inquiry_items supplier_quotes customer_quotes conversations attachments buyer_profiles priority_rules assignment_rules assignment_accept_tokens manager_approvals; do
